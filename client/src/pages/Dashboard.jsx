...
const StatCard = ({ title, val, trend, icon: Icon, bgLight }) => (
  <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 hover:shadow-md transition-all relative overflow-hidden group">
    <div className="flex justify-between items-start mb-4">
      <div className={`p-3.5 rounded-xl ${bgLight} text-slate-700 group-hover:scale-105 transition-transform duration-200`}>
    ...
    <div className="p-6 lg:p-8 bg-slate-50 min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        ...
        <div className="flex items-center space-x-3">
          <Link
            to="/leads"
            className="bg-purple-600 hover:bg-purple-700 text-white font-semibold text-xs sm:text-sm px-4 py-2.5 rounded-xl shadow-md flex items-center transition-all"
          >
            <Plus className="w-4 h-4 mr-1.5" /> Add Lead
          </Link>
          <Link
            to="/properties"
            className="bg-slate-900 hover:bg-slate-800 text-white font-semibold text-xs sm:text-sm px-4 py-2.5 rounded-xl flex items-center transition-all shadow-sm"
          >
            <Building className="w-4 h-4 mr-1.5" /> New Property
          </Link>
        </div>
      </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 flex flex-col">
          ...
          <div className="space-y-4 flex-1">
            {recentActivities.map((act, i) => (
              <div key={i} className="p-3.5 bg-slate-50 rounded-xl border border-slate-100 flex items-start space-x-3">
                <div className="p-2 bg-slate-200/80 rounded-lg text-slate-700 shrink-0 mt-0.5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center space-x-2 mt-1">
                    ...
                    <span className="text-[9px] px-2 py-0.5 bg-slate-200 text-slate-700 font-bold rounded-full">
                      {act.tag}
                    </span>
                  </div>
                </div>
